    ...
    <div class="py-8 px-4 sm:px-10 border-[#1a936f] ">
        ...
        <div class="mt-8 flex flex-col px-4 sm:px-6 lg:px-8">
            <div class="-my-2 -mx-4 sm:-mx-6 lg:-mx-8">
                <table class="min-w-full d-table border-separate border-spacing-y-1" id="user_table">
                        ...
                        @foreach($roles as $key => $role)
                            <tr class="shadow-md rounded-full ">
                                ...
                                <td class="border-r-2 rounded-lg border-gray-300 bg-gray-50 bg-opacity-75 py-1.5 p-4 text-left text-sm text-gray-900 backdrop-blur backdrop-filter sm:pl-6 lg:pl-8">
                                    <div class="flex">
                                    <a data-id="{{ $role->id }}" class="editModal inline-flex items-center justify-center rounded-md border border-transparent bg-[#1a936f] p-2 text-sm font-bold shadow-md hover:bg-[#114d3b] hover:text-white focus:outline-none focus:ring-2 focus:ring-[#1a936f] focus:ring-offset-2 sm:w-auto mr-2" >
                                    ...
                                    <a data-id="{{ $role->id }}" data-url="{{ route('roles.destroy', $role->id) }}" class="deleteModal inline-flex items-center justify-center rounded-md border border-transparent bg-rose-700 p-2 text-sm font-bold shadow-md hover:bg-rose-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-2 sm:w-auto mr-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-white hover:text-white">
                ...
                <div class="card border border-cyan-700 rounded ">
                    <div class="card-header border-b-2">
                        <div class="flex px-4 py-2 justify-between items-center">
                            <h3 class="font-bold text-[#1a936f]">Create Role</h3>
                                        ...
                                        @foreach($permissions as $permission)
                                            <div class="relative flex items-start col-span-1 mx-1">
                                                <div class="flex h-5 items-center">
                                                    <input aria-describedby="permission-description" name="permissions[]" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-[#1a936f] focus:ring-cyan-500" value="{{ $permission->id }}" id="permission_{{ $permission->id }}">
                                                </div>
